...
import { getContractInterface, ChugSplashAction, isSetStorageAction } from '@eth-optimism/contracts'

import { OptimismEnv } from './shared/env'
import { l1Provider, l2Provider, OVM_ETH_ADDRESS } from './shared/utils'
import newExecutionManagerJson from '../artifacts/contracts/ovm-self-upgrades/NewExecutionManager.sol/NewExecutionManager.json'

describe.only('OVM Self-Upgrades', async () => {
  let env: OptimismEnv
  let l2Wallet: Wallet
  let OVM_UpgradeExecutor: Contract // TODO: replace with L2ChugsplashDeployer (or maybe owner actually?)
  let Factory__ReturnOne: ContractFactory
  let DeployedBytecode__ReturnTwo: string
  let Factory__SimpleStorage: ContractFactory

  // TODO: gotta update this function to approve and execute bundles.
  // Steps: 1) use getChugSplashActionBundle(instructions) 2) figure out how to get control over the right owner in integration tests 3) 
  const applyChugsplashInstructions = async (
    instructions: ChugSplashAction[]
  ) => {
    // TODO: const bundle = getChugSplashActionBundle(instructions)
    // TODO: get the owner into the integration test and approve the bundle hash
    for (const instruction of instructions) {
      // TODO: execute action through the L2ChugsplashDeployerOwner
      let res
      if (isSetStorageAction(instruction)) {
  ...
  before(async () => {
    ...
    const returnTwo: Contract = await Factory__ReturnTwo.deploy()
    await returnTwo.deployTransaction.wait()
  ...
  describe('setStorage and setCode are correctly applied according to geth RPC', () => {
    it('Should execute a basic storage upgrade', async () => {
      const basicStorageUpgrade: ChugSplashAction[] = [
        {
    ...
    it('Should execute a basic upgrade overwriting existing deployed code', async () => {
      ...
      const basicCodeUpgrade: ChugSplashAction[] = [
        {
    ...
    it('Should execute a basic code upgrade which is not overwriting an existing account', async () => {
      const emptyAccountCodeUpgrade: ChugSplashAction[] = [
        {
  ...
  describe('Contracts upgraded with setStorage and setCode behave as expected', () => {
    it('code with updated storage returns the new storage', async () => {
      ...
      const storageVarUpgrade: ChugSplashAction[] = [
        {
    ...
    it('code with an updated constant returns the new constant', async () => {
      ...
      const constantUpgrade: ChugSplashAction[] = [
        {
  ...
  describe('L2 virtualization contracts can themselves be upgraded', async () => {
    it('does a basic upgrade to the Execution Manager', async () => {
      const executionManagerUpgrade: ChugSplashAction = {
        ...
        code: newExecutionManagerJson.deployedBytecode
      }

      await applyAndVerifyUpgrade([executionManagerUpgrade])
    })
  })
})
